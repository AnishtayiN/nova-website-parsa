...
/**
 * لاگ عملیات پنل مدیریت
 */
function log_admin(string $message): void
{
    log_error("[ADMIN] $message");
}
...
<?php if (!$isAuth): ?>
  ...
  <!-- ═══ صفحه ورود (بدون نمایش هیچ اطلاعات ورود) ═══ -->
  <div class="login-wrap" id="loginView">
    <div class="login-card" id="loginCard">
      ...
      <form id="loginForm" novalidate>
        <div class="field">
          <label for="loginUser">نام کاربری</label>
          <input type="text" id="loginUser" placeholder="نام کاربری" autocomplete="username" required />
        </div>
        <div class="field">
          <label for="loginPass">رمز عبور</label>
          <input type="password" id="loginPass" placeholder="رمز عبور" autocomplete="current-password" required />
        </div>
...
<?php else: ?>
  ...
  <div class="admin-app" id="adminApp">
    ...
    <main class="main">
      <header class="topbar">
        <div>
          <h2 id="viewTitle">داشبورد</h2>
          <p class="topbar-sub" id="viewSub">نمای کلی وضعیت سایت</p>
        </div>
      ...
      <section class="view active" id="view-dashboard">
        ...
        <div class="panel">
          <div class="panel-head">
            <h3>🆕 آخرین پیام‌ها</h3>
            <button class="mini-btn" data-goto="messages" type="button">مشاهده همه ←</button>
          </div>
      ...
      <section class="view" id="view-content">
        <div class="panel form-panel">
          ...
          <div class="form-grid">
            <div class="field"><label for="cfTitle">عنوان سایت (تب مرورگر)</label><input id="cfTitle" type="text" value="<?= e($settings['title']) ?>" /></div>
            ...
            <div class="field full"><label for="cfWords">کلمات افکت تایپ (با ویرگول جدا کنید)</label><input id="cfWords" type="text" value="<?= e(implode(', ', $settings['words'])) ?>" /></div>
          </div>

          <h3 style="margin-top:28px">📞 اطلاعات تماس (بخش تماس سایت)</h3>
          <div class="form-grid">
          ...
          <p class="form-note" style="margin-top:12px">💡 تغییرات در دیتابیس JSON ذخیره می‌شوند و بلافاصله روی صفحه اصلی اعمال می‌شوند.</p>
        </div>
      </section>
      ...
      <section class="view" id="view-settings">
        <div class="panel form-panel">
          ...
          <div class="form-grid">
            ...
            <div class="field"><label for="setPass1">رمز جدید (حداقل ۸ حرف، شامل حرف انگلیسی و رقم)</label><input id="setPass1" type="password" /></div>
        ...
        <div class="panel form-panel">
          <h3>👤 نام کاربری</h3>
          <div class="form-grid">
            <div class="field"><label for="setUser">نام کاربری جدید (۳ تا ۲۰ حرف انگلیسی)</label><input id="setUser" type="text" value="<?= e($adminUser) ?>" /></div>
          </div>
          <div class="form-actions"><button class="btn-save" id="saveUser" type="button">اعمال</button></div>
        </div>

        <div class="panel form-panel">
          <h3>🎨 حالت نمایش پیش‌فرض سایت</h3>
          <div class="form-grid">
